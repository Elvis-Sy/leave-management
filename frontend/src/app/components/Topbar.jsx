          ...
          <PopoverTrigger className="flex items-center gap-2 bg-[#f1f1f1] py-2 px-2 rounded-xl">
            <div className="cursor-pointer">
              <div className="flex flex-col gap-1">
                {infoUser.name && <span className="text-[12px] leading-3 font-medium text-gray-900">{infoUser.name}</span>}
                <div className="flex justify-end">
                  <span className="text-xs text-white px-2 rounded-full bg-gray-500">{role}</span>
                </div>
              </div>
              <img src={`http://localhost:5000/${infoUser.photo}`} alt="" width={30} height={30} className=" w-8 h-8 rounded-full object-cover"/>
            </div>
          </PopoverTrigger>
